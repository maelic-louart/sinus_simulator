...
typedef struct {
    ...
    XSinus_simulator_uio_map maps[ MAX_UIO_MAPS ];
} XSinus_simulator_uio_info;

/***************** Variable Definitions **************************************/
static XSinus_simulator_uio_info uio_info;
...
static int uio_info_read_name(XSinus_simulator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSinus_simulator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSinus_simulator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSinus_simulator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSinus_simulator_Initialize(XSinus_simulator *InstancePtr, const char* InstanceName) {
	XSinus_simulator_uio_info *InfoPtr = &uio_info;
